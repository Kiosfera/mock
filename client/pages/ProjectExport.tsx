...
export default function ProjectExport() {
      ...
      const interval = setInterval(() => {
        if (currentFile <= exportStatus.totalFiles) {
          ...
          setExportStatus(prev => ({
            ...
            estimatedTime: Math.floor(((Date.now() - startTime) / currentFile * prev.totalFiles - (Date.now() - startTime)) / 1000),
            errors: !success ? [...prev.errors, `Erro ao exportar peça ${currentFile}`] : prev.errors
          }));
  ...
  const getFormatInfo = (format: string) => {
    switch (format) {
      case "jpeg": return { label: "JPEG", description: "Menor tamanho, boa qualidade", icon: Image };
      case "png": return { label: "PNG", description: "Transparência, maior qualidade", icon: Image };
      case "pdf": return { label: "PDF", description: "Vetorial, ideal para impressão", icon: FileText };
      case "svg": return { label: "SVG", description: "Vetorial, escalável", icon: FileText };
      default: return { label: format.toUpperCase(), description: "", icon: FileImage };
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900 font-poppins">Exportação para Produção</h1>
            ...
            <div className="flex items-center space-x-3">
              {exportStatus.status === "completed" && selectedFiles.length > 0 && (
                <Badge variant="outline">
                  {selectedFiles.length} selecionado{selectedFiles.length !== 1 ? 's' : ''} ({getFileSizeTotal()} MB)
                </Badge>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="border-0 shadow-md">
              <CardHeader>
                <CardTitle className="text-lg font-poppins">Configurações</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value: "jpeg" | "png" | "pdf" | "svg") => updateSetting('format', value)}
                  >
                ...
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => updateSetting('resolution', parseInt(value))}
                  >
                    <SelectTrigger className="mt-2">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateSetting('includeNames', checked)}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateSetting('createZip', checked)}
                    />
                  </div>
                </div>
              ...
              <Card className="border-0 shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg font-poppins flex items-center space-x-2">
                    {exportStatus.status === "preparing" && <Clock className="w-5 h-5 text-blue-500" />}
                    {exportStatus.status === "exporting" && <RefreshCw className="w-5 h-5 text-blue-500 animate-spin" />}
                    {exportStatus.status === "completed" && <CheckCircle className="w-5 h-5 text-green-500" />}
                    {exportStatus.status === "error" && <AlertCircle className="w-5 h-5 text-red-500" />}
                    <span className="capitalize">{exportStatus.status === "preparing" ? "Preparando" : exportStatus.status === "exporting" ? "Exportando" : exportStatus.status === "completed" ? "Concluído" : "Erro"}</span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  {exportStatus.isExporting && (
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs text-gray-500">
                        <span>Arquivo {exportStatus.currentFile} de {exportStatus.totalFiles}</span>
                        <span>{exportStatus.elapsedTime}s / ~{exportStatus.estimatedTime}s</span>
                      </div>
                    </div>
                  )}
                  ...
                  {exportStatus.status === "completed" && (
                    <div className="space-y-3">
                      <div className="text-center p-3 bg-green-50 rounded-lg">
                        <div className="text-lg font-bold text-green-600">{exportStatus.exportedFiles.length}</div>
                        <div className="text-green-700 text-sm">Arquivos Exportados</div>
                      </div>
                      
                      {exportStatus.errors.length > 0 && (
                        <div className="text-center p-3 bg-red-50 rounded-lg">
                          <div className="text-lg font-bold text-red-600">{exportStatus.errors.length}</div>
              ...
              <>
                ...
                <Card className="border-0 shadow-md">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-lg font-poppins">Arquivos Exportados</CardTitle>
                      <div className="flex items-center space-x-2">
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={selectedFiles.length === exportStatus.exportedFiles.length ? clearSelection : selectAllFiles}
                        >
                          {selectedFiles.length === exportStatus.exportedFiles.length ? "Desmarcar Todos" : "Selecionar Todos"}
                        </Button>
                        <Button 
                          variant="outline" 
                          size="sm"
                          ...
                          {exportStatus.exportedFiles.map((file, index) => (
                            <div 
                              ...
                              className={`p-4 hover:bg-gray-50 transition-colors flex items-center space-x-4 ${
                                index !== exportStatus.exportedFiles.length - 1 ? 'border-b border-gray-100' : ''
                              }`}
                            >
                              <Checkbox 
                                checked={selectedFiles.includes(file.id)}
              ...
              <Card className="border-0 shadow-md">
                <CardContent className="p-12">
                  <div className="text-center space-y-6">
                    ...
                    <div className="space-y-2">
                      <h2 className="text-2xl font-bold text-gray-900 font-poppins">Pronto para Exportar</h2>
                      <p className="text-gray-600 max-w-md mx-auto">
                        Configure as opções de exportação ao lado e clique em "Iniciar Exportação" para gerar os arquivos de produção.
                      </p>
                    </div>
                    <div className="space-y-4">
                      <p className="text-sm text-gray-500">Formatos disponíveis:</p>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                        {["jpeg", "png", "pdf", "svg"].map((format) => {
                          const info = getFormatInfo(format);
                          const Icon = info.icon;
                          return (
                            <div key={format} className="text-center p-3 bg-gray-50 rounded-lg">
